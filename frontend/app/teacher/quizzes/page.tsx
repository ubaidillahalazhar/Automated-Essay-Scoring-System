...
import {
  CheckCircle2, Heart, MessageCircle, Search, Pencil, Trash2,
  Loader2, AlertTriangle, X, PlusSquare
} from "lucide-react"
import styles from "@/styles/teacher-dashboard.module.css"
import { apiFetch } from "@/lib/api"

const LS_GRADE_FILTER_KEY = "teacher_quizzes_grade_filter"
...
export default function TeacherQuizzes() {
    ...
    async function loadData() {
      ...
      try {
        const [quizzesRes, subjectsRes, gradesRes] = await Promise.all([
          ...
          apiFetch(`/api/grades?level=${teachingLevel}`),
        ])
  ...
  async function handleConfirmDelete() {
    ...
    setDeleting(true)
    try {
      const res = await apiFetch(`/api/exams/${deleteTarget.id}`, { method: "DELETE" })
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.body}>
        <main className={styles.main}>
          ...
          {/* Page header: kiri = title "Kuis Saya", kanan = tombol Buat Kuis */}
          <div className={styles.pageHeader}>
            <div className={styles.pageTitleRow}>
              <h1 className={styles.pageTitle}>Kuis Saya</h1>
            </div>
